package com.guoyoujin.gz.gz.weath;

import android.text.TextUtils;
import android.util.Log;

import com.guoyoujin.gz.gz.application.MyApplication;
import com.guoyoujin.gz.gz.utils.FileUtils;
...
public class ConfigCache {
	...
	public static String getUrlCache(String url) {
		...
		if (file.exists() && file.isFile()) {
			...
			//如果是wifi网络，则30分钟过期
			if (MyApplication.mNetWorkState == NetUtil.NETWORN_WIFI
					&& expiredTime > CONFIG_CACHE_WIFI_TIMEOUT) {
				return null;
				//如果是手机网络，则2个小时过期
			} else if (MyApplication.mNetWorkState == NetUtil.NETWORN_MOBILE
	...
	public static void setUrlCache(String data, String url) {
		if (MyApplication.getInstance().getCacheDir() == null) {
			return;
		}
